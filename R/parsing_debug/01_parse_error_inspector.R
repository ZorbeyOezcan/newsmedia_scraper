...
parse_html_local <- local({
  ...
  # Hilfsfunktion zum Laden der Regeln
  ...
  # Hilfsfunktion: Prüft CSS-Selektoren im HTML (aus 06_html_parser.R übernommen)
  .check_paywall_markers_local <- function(html_parsed, paywall_markers) {
    if (length(paywall_markers) == 0) return(FALSE)
    
    # Prüfe CSS Selektoren
    if (!is.null(paywall_markers$css_selectors)) {
      for (pattern in paywall_markers$css_selectors) {
        tryCatch({
          # Doppelpunkte in Selektoren escapen, falls nötig
          escaped_pattern <- gsub(":", "\\\\:", pattern)
          if (length(rvest::html_nodes(html_parsed, escaped_pattern)) > 0) return(TRUE)
        }, error = function(e) {
          return(FALSE)
        })
      }
  ...
  function(data_table_input, force_reload = FALSE) {
    ...
    if (is.character(data_table_input)) {
      ...
      if (!exists(data_table_name, envir = .GlobalEnv)) stop("Data.table not found.")
    ...
    message(sprintf("--- Locally parsing %d entries from '%s' for domain '%s' (with Paywall Check) ---", nrow(source_dt), data_table_name, domain_clean))
    
    # Regeln für die Domain holen
    parser_entry <- .local_rules_cache$parser[[domain_clean]]
    paywall_entry <- .local_rules_cache$paywall[[domain_clean]]
    ...
    for (i in 1:nrow(source_dt)) {
      ...
      if (!is.null(html_parsed)) {
        ...
        # 2. Paywall Regeln prüfen (NEU HINZUGEFÜGT)
        # Zuerst schauen, ob der Parser selbst etwas gefunden hat
        from_parser <- if (!is.null(extracted_data$paywall)) as.logical(extracted_data$paywall) else NA
        
        # Dann die CSS-Marker prüfen
        from_markers <- FALSE
        if (!is.null(paywall_entry) && isTRUE(paywall_entry$has_paywall)) {
          from_markers <- .check_paywall_markers_local(html_parsed, paywall_entry$paywall_markers)
        }
        
        # Logik: Wenn Marker gefunden ODER Parser sagt ja -> Paywall ist TRUE
        if (isTRUE(from_markers) || isTRUE(from_parser)) {
          parsed_data$paywall_detected <- TRUE
        } else {
          parsed_data$paywall_detected <- FALSE
        }
      }
      ...
      results_list[[i]] <- data.table(
        ...
        paywall = parsed_data$paywall_detected # Hier nutzen wir jetzt das ermittelte Ergebnis
      )
    ...
    print(head(result_dt[, .(url, paywall)])) # Zeige direkt Paywall-Status
    invisible(TRUE)
  }
})
